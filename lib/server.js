...
var authenticationStrategies = null;
...
function configure(confPath, done) {
  fs.readFile(path.join(__dirname, '..', 'default.config.yaml'), 'utf8', function(error, defaults) {
    ...
    ConfigLoader.load(defaults, confPath, function(error, localConfig) {
      ...
      for (i in localConfig.authenticationStrategies) {
        var strategy = localConfig.authenticationStrategies[i];
        // TODO: Allow global plugin includes?
        // TODO: Support an async attach?
        var requirePath = path.join(__dirname, 'plugins', i);
        authenticationStrategies[i] = require(requirePath).attach(passport, app, strategy, logger);
      }
 ...
 * Start the server(s).
 */
function start(confPath, done) {
  // confPath is optional.
  ...
  if (confPath !== null) {
    actions.push(configure.bind(null, confPath));
  }
...
function proxyRoute(req, res, next) {
  ...
  if (route) {
    ...
    var user = (req.user && req.user._json && req.user._json.email) ? req.user._json.email : 'Anonymous';
