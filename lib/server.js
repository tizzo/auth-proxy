...
// Set app defaults.
var config = JSON.parse(fs.readFileSync('default.config.json'));
...
function configure(confPath, done) {
  var localConfig = ConfigLoader.load(confPath);
  var routes = [];
...
function start(confPath, done) {
  ...
  var actions = [
    configure.bind(null, confPath),
  ];

  actions.push(server.listen.bind(server, config.port));

  if (config.httpPort) {
    actions.push(httpServer.listen.bind(httpServer, config.httpPort));
  }
