...
function configure(confPath, done) {
  fs.readFile(path.join(__dirname, '..', 'default.config.yaml'), 'utf8', function(error, defaults) {
    ...
    ConfigLoader.load(defaults, confPath, function(error, localConfig) {
      ...
      for (name in localConfig.authenticationStrategies) {
        var strategyConfig = localConfig.authenticationStrategies[name];
        // If we do not ship with a plugin by the name listed in the configuration,
        // try to require it.
        authenticationStrategies[name] = PassportPlugins[name] || require(name);
        authenticationStrategies[name].attach(passport, app, strategyConfig, logger);
      }
      localConfig.routes = routes;
      module.exports.config = config = localConfig;
      // Clear the configured express routes and reattach them from the configuration.
      app.routes = {};
