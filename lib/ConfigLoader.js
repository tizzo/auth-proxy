 ...
 */
module.exports.load = function(defaultConfig, confPath, done) {
  var tasks = [
    function(cb) {
  ...
  if (typeof confPath === 'string') {
    tasks.push(function(config, cb) {
      fs.exists(confPath, function(exists) {
        if (exists) {
          fs.readFile(confPath, 'utf8', function(error, data) {
            config = mergeConifguration(config, yaml.safeLoad(data));
            return cb(null, config);
          });
        }
        else {
          cb(new Error('Specified configuration file not found.'));
        }
      });
    });
  }
