...
AuthProxy.prototype.configure = function configure(localConfig, done) {
  ...
  for (name in self.config.authenticationStrategies) {
    ...
    self.authenticationStrategies[name] = PassportPlugins[name] || require(name);
    self.authenticationStrategies[name].attach(passport, app, self.config, strategyConfig, logger);
  }
